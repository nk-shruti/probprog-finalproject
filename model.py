...
def model_spatio_temporal_linear(data_x, data_y, node1, node2, D, months):
    beta = []
    for i in range(D + 1):
        beta.append(pyro.sample("beta" + str(i), dist.Normal(0., 10.)))
    ...
    phi = []
    for i in range(2078):
        p = pyro.sample("phi" + str(i), dist.Normal(0., 10.))
        phi.append(p)
        sum_p += p
    phi.append(-sum_p)
    ...
    delta = []
    for i in range(2078):
        de = pyro.sample("delta" + str(i), dist.Normal(0., 10.))
        delta.append(de)
        sum_delta += de
...
def model_spatio_temporal_linear_one_icar(
        data_x, data_y, node1, node2, D, months):
    beta = []
    for i in range(D + 1):
        beta.append(pyro.sample("beta" + str(i), dist.Normal(0., 1.)))
...
def guide_spatio_temporal_linear_one_icar(
        data_x, data_y, node1, node2, D, months):
    mu_i = []
    sigma_i = []
    for i in range(D + 1):
