...
complaint_locations = ['Latitude', 'Longitude',
                       'ADDR_PCT_CD', 'BORO_NM', 'PATROL_BORO']
crime_details = ['KY_CD', 'PD_CD', 'LAW_CAT_CD']
# ,'OFNS_DESC', 'CRM_ATPT_CPTD_CD', 'PD_DESC']
...
df_data = df[
    complaint_num + complaint_date + complaint_locations + crime_details
    + surrounding_desc + suspect_data + victim_data]

# blocks['Tract'] = blocks.BlockCode // 10000
# The block code takes the form
# XXYYYZZZZZZAAAA where X=State, Y=County, Z=Tract, A=Block.
...
row = df_ds1.apply(lambda row: int(row['BlockCode']) // 10000, axis=1)
df_ds1['CensusTract'] = row
